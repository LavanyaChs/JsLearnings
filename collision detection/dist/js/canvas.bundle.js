...
function resolveCollision(particle, otherParticle) {
  ...
  if (xVelocityDiff * xDist + yVelocityDiff * yDist >= 0) {
    ...
    var v2 = {
      x: u2.x * (m2 - m2) / (m1 + m2) + u1.x * 2 * m2 / (m1 + m2),
      y: u2.y
...
function init() {
  objects = []; // circleObj = new Object(300,300,100,'black');
  ...
  for (var i = 0; i < 300; i++) {
    ...
    if (i != 0) {
      for (var j = 0; j < objects.length; j++) {
        var dist = getDistance(x, y, objects[j].x, objects[j].y);
    ...
    objects.push(new _Object(x, y, radius, color));
  }
} // Animation Loop
